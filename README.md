...
큐는 agent가 선언해 들고 있는 객체이지만, 큐에 넣는 일은 타깃 앱의 요청 스레드가 직접 수행합니다. agent의 후킹 코드가 그 요청 스레드에서 함께 실행되기 때문입니다. ArrayBlockingQueue의 단일 lock에 경쟁이 생기면 lock을 기다리는 주체가 곧 타깃 앱의 요청 스레드라, 그 대기 시간만큼 타깃 앱의 요청 처리가 지연되는 것이 agent 모듈의 특수성입니다.

이 특수성을 줄이려면 lock 경쟁이 생기는 자리를 먼저 봐야 하는데, 큐에 걸리는 부하는 두 가지입니다. 하나는 큐에서 꺼내 gateway로 보내는 전송 부하이고, 다른 하나는 후킹 데이터가 큐로 흘러드는 인입 부하입니다. 이 중 타깃 앱의 요청 스레드가 직접 맡는 쪽은 인입 부하이므로, 타깃 앱 지연에 직결되는 것은 데이터를 넣는 producer의 lock 경쟁입니다.
...
이 지연을 지표로 삼은 이유는 앱 간섭에 있습니다. 큐에 적재하는 producer는 타깃 앱의 요청 스레드이고, 적재 한 건에 걸리는 시간이 그대로 타깃 앱의 요청 처리 시간에 더해집니다. 큐 성능을 재는 지표에는 적재 지연 말고 처리량(throughput)도 있는데, 처리량은 큐로 데이터를 한계까지 몰아넣어 큐가 언제 가득 차는지를 측정합니다. agent는 앱 하나마다 큐 하나로 붙고, 큐에 넣는 일은 앱이 본래 돌리던 요청 스레드가 하므로 동시에 들어오는 양이 그 앱의 동시 요청 수에 묶입니다. 요청 사이에는 간격이 있어 측정 환경처럼 큐를 극한 속도로 채울 일도 드뭅니다. 그래서 처리량을 한계까지 밀어붙이는 상황은 실제 agent에서 잘 일어나지 않고, 처리량을 재도 agent의 실제 동작을 설명하기 어렵습니다. 이런 이유로 처리량 측정은 agent에 맞는 큐를 고르는 근거로 삼기에 적합하지 않다고 판단했습니다.
