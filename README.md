...
이 프로젝트의 agent는 모니터링 대상 앱 하나마다 하나씩 붙어 앱 프로세스(JVM) 안에서 동작합니다. 앱의 여러 요청 스레드가 후킹 지점을 지나며 수집한 span을 하나의 큐에 인입하고, 큐에서 span을 꺼내 gateway로 보내는 일은 QueueWorker라는 단일 객체 하나가 맡습니다.
...
큐는 agent가 선언해 들고 있는 객체이지만, 큐에 넣는 일은 타깃 앱의 요청 스레드가 직접 수행합니다. agent의 후킹 코드가 그 요청 스레드에서 함께 실행되기 때문입니다. ArrayBlockingQueue의 단일 lock에 경쟁이 생기면 lock을 기다리는 스레드가 곧 타깃 앱의 요청 스레드라, 그 대기 시간만큼 타깃 앱의 요청 처리가 밀립니다. ArrayBlockingQueue를 쓰면 이 대기 시간이 그대로 타깃 앱의 비용이 됩니다.

이 비용을 줄이려면 lock 경쟁이 어디서 생기는지부터 봐야 합니다. 큐에서 꺼내 gateway로 보내는 전송은 단일 consumer가 맡지만, 후킹 데이터를 넣는 인입은 타깃 앱의 요청 스레드가 직접 합니다. 그렇기 때문에 producer가 데이터를 넣으며 겪는 lock 경쟁이 곧 타깃 앱의 지연이 될 수 있습니다.

이 프로젝트에서는 producer 쪽 부하가 높지 않아 ArrayBlockingQueue로도 충분히 동작합니다. 다만 후킹 포인트를 크게 확장하면 한 요청을 처리하는 동안 큐에 넣는 횟수가 늘고, producer가 같은 lock을 두고 벌이는 경쟁이 커지면서 그만큼 타깃 앱의 요청 처리도 지연됩니다.
...
이 지연을 지표로 삼은 것은 앱 간섭 때문입니다. 큐에 적재하는 producer는 타깃 앱의 요청 스레드이고, 적재 한 건에 걸리는 시간이 그대로 타깃 앱의 요청 처리 시간에 더해집니다. 큐 성능을 재는 지표에는 적재 지연 말고 처리량(throughput)도 있는데, 처리량은 큐로 데이터를 한계까지 몰아넣어 큐가 언제 가득 차는지를 측정합니다. agent는 앱 하나마다 큐 하나로 붙고, 큐에 넣는 일은 앱이 본래 돌리던 요청 스레드가 하므로 동시에 들어오는 양이 그 앱의 동시 요청 수에 비례합니다. 요청 사이에는 간격이 있어 측정 환경처럼 큐를 극한 속도로 채울 일도 드뭅니다. 그래서 처리량을 한계까지 밀어붙이는 상황은 실제 agent에서 잘 일어나지 않고, 처리량을 재도 agent의 실제 동작을 설명하기 어렵습니다. 이런 이유로 처리량 측정은 agent에 맞는 큐를 고르는 근거로 삼기에 적합하지 않다고 판단했습니다.

적재 지연은 여러 적재의 지연을 평균낸 평균과, 그 지연들이 평균에서 얼마나 멀어졌는지를 보는 변동 두 가지로 구분했습니다. lock 큐에서는 대부분의 적재가 빠르게 끝나는 가운데, 가끔 lock 경쟁에 걸린 한 건이 평균보다 훨씬 오래 걸립니다. 이렇게 오래 걸리는 적재는 드물어 평균에는 잘 드러나지 않지만, 그 순간 타깃 앱의 응답 한 건이 크게 늦어집니다. 평균이 낮아도 이런 큰 지연이 가끔 섞이면 타깃 앱에 영향을 주므로, 변동을 평균과 함께 봤습니다.
...
![큐 벤치마크 테스트 구조](docs/benchmark-queue.png)
...
분석 대상 구간의 지연 차이는 offer 한 번에 약 40ns입니다. 큐에 데이터를 넣는 일은 타깃 앱 위에서 도는 agent에서, 타깃 앱이 요청을 처리하는 동안 일어납니다. 요청 처리는 보통 밀리초 단위라, 큐 성능을 재는 나노초 단위의 차이는 실질적으로 체감하기 어렵습니다. 측정 결과 MpscArrayQueue는 처리 지연이 작고 일정해, 다수의 인입 경로와 단일 소비 경로 구조에서 이점이 있습니다.
...
MpscArrayQueue는 JCTools 라이브러리가 제공하는 lock-free 큐입니다. [JCTools 공식 문서](https://javadoc.io/static/org.jctools/jctools-core/3.3.0/org/jctools/queues/MpscArrayQueue.html)는 어느 스레드든 offer를 호출할 수 있지만 poll과 peek은 한 스레드만 호출해야 정확성이 유지된다고 명시합니다. 이 조건이 곧 MPSC, 다수 producer와 단일 consumer 구조입니다. 이 구조는 agent의 큐 구조와 같아, 이 사례에서 ArrayBlockingQueue보다 MpscArrayQueue가 더 적합한 자료구조라고 생각했습니다.
